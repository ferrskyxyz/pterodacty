    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("© FerrStoree Foundation Protect\n\nData Hanya Bisa Diubah Oleh Admin ID 1");
            }
        }

        // Cegah turunkan level admin ke user biasa
        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("© FerrStoree Foundation Protect\n\nAkses Ditolak : Kamu Tidak Dapat Menurunkan Admin Orang Lain Kecuali ( Admin ID 1 )");
        }
        // ====================================================
